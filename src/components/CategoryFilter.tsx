...
const CATEGORIES: { id: BotCategory; label: string; icon: React.ReactNode }[] = [
  ...
  { id: 'AI', label: 'AI & Chatbot', icon: <BotIcon className="w-3.5 h-3.5" /> },
  { id: 'ANON_CHAT', label: 'Anon Chat', icon: <MessageSquare className="w-3.5 h-3.5" /> },
  { id: 'GAME', label: 'Mini Apps/Game', icon: <Gamepad2 className="w-3.5 h-3.5" /> },
...
export const CategoryFilter: React.FC<CategoryFilterProps> = ({
  ...
  return (
    <div className="flex items-center justify-center my-6" id="kategori">
      ...
      <div className="inline-flex items-center p-1.5 rounded-full bg-white border border-slate-200/90 shadow-xs overflow-x-auto no-scrollbar max-w-full">
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-xs transition-all whitespace-nowrap cursor-pointer ${activeClass}`}
            >
              {cat.icon}
              <span>{cat.label}</span>
            </button>
