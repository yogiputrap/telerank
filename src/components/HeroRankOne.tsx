...
export const HeroRankOne: React.FC<HeroRankOneProps> = ({ bot, onOutbidRankOne }) => {
  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto my-6 px-4">
      ...
      <div className="absolute -inset-1 bg-gradient-to-r from-amber-500/20 via-yellow-500/30 to-amber-600/20 rounded-3xl blur-2xl opacity-70 pointer-events-none" />

      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-b from-slate-900/95 via-[#111927] to-[#0f172a] border-2 border-amber-500/50 gold-glow p-6 sm:p-8 md:p-10 shadow-2xl">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left flex-1">
            <div className="relative group shrink-0">
              ...
              {/* Crown Badge */}
              <div className="absolute -top-3.5 -right-3.5 bg-gradient-to-tr from-amber-500 to-yellow-300 text-slate-950 p-1.5 rounded-full shadow-lg border border-amber-200 animate-bounce">
                <Crown className="w-5 h-5 fill-slate-950" />
              </div>
              {/* Rank #1 Chip */}
              <div className="absolute -bottom-2.5 left-1/2 transform -translate-x-1/2 px-3 py-0.5 rounded-full bg-gradient-to-r from-amber-500 to-yellow-400 text-slate-950 font-black text-xs uppercase tracking-widest shadow-md flex items-center gap-1">
            ...
            <div className="space-y-2 max-w-xl">
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                <span className="px-2.5 py-0.5 rounded-full bg-amber-500/15 border border-amber-500/40 text-amber-300 text-xs font-bold flex items-center gap-1">
                  <Flame className="w-3.5 h-3.5 fill-amber-400 text-amber-400" /> Raja Papan Peringkat
                </span>
                <span className="px-2.5 py-0.5 rounded-full bg-sky-500/15 border border-sky-500/30 text-sky-300 text-xs font-medium">
                  Kategori: {bot.category}
                </span>
                {bot.is_verified && (
                  <span className="px-2 py-0.5 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 text-[11px] font-medium flex items-center gap-1">
